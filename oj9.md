# oj赛第九周|MT3011骑士锦标赛、MT3012序列加法、3.剪纸、MT3015构造原理、MT3014小码妹排序
...
4. 几种取反操作
``` python3
 original_list = [0, 1, 1, 0, 1, 0, 0, 1]
 #使用列表推导式将所有的0变为1，1变为0
 inverted_list1 = [1 - x for x in original_list]
 #使用列表推导式和not运算符进行转换
inverted_list = [int(not x) for x in original_list]
```
